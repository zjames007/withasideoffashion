...
if ($title !== '') {
	if ( $a_href !== '' && $linked_title !== '' )
		$title = '<a href="'. esc_url($a_href).'"'.$a_title.$a_target.$a_rel.$lightbox_data_title.'>' . $title . '</a>';
...
if ($link_text !== '' && $a_href !== '') $output_text .= '<p class="text-bold"><a class="btn btn-link" href="'. esc_url($a_href).'"'.$a_title.$a_target.$a_rel.'>' . $link_text . '</a></p>' ;

if ($output_text !== '') $output_text = '<div class="icon-box-content' . esc_attr( $add_margin ) . '">' . $output_text . '</div>';

// Prepare icon area
$tag_start = ($a_href !== '') ? 'a href="'. esc_url($a_href) .'"'.$a_title.$a_target.$a_rel.$lightbox_data : 'span';
